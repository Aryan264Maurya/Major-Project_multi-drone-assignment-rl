"""
ppo_assignment.py  (updated – higher entropy + longer rollout for convergence)
=================
Key changes vs previous version:
  - entropy_coef  : 0.01  → 0.05   (more exploration)
  - clip_range    : 0.2   → 0.25   (wider clip for early exploration)
  - update_every  : 64    → 256    (longer rollout before each PPO update)
  - lr            : 3e-4  → 1e-4   (slower, more stable convergence)
...
def train_ppo(model_path="ppo_assignment.pt", episodes=10_000,
    ...
    print(f"PPO config → entropy=0.05  clip=0.25  rollout={update_every}  lr=1e-4  mb=64")
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            if len(agent.buffer) >= update_every:
                m = agent.update()
                if m: history["loss"].append(m["actor_loss"]+m["value_loss"]-m["entropy"])
