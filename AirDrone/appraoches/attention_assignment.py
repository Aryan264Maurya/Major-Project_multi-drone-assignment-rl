"""
attention_assignment.py  (updated – higher entropy + longer rollout for convergence)
========================
Key changes vs previous version:
  - entropy_coef  : 0.01  → 0.05
  - clip_range    : 0.2   → 0.25
  - update_every  : 64    → 256   (longer rollout)
  - lr            : 3e-4  → 1e-4
...
class AttentionAgent:
    def __init__(self, n, m, d_model=128, n_heads=4, n_layers=3,
                 lr=1e-4,           # ← 3e-4 → 1e-4
                 gamma=0.99,
                 clip_range=0.25,   # ← 0.2  → 0.25
                 entropy_coef=0.05, # ← 0.01 → 0.05
                 value_coef=0.5,
                 update_epochs=10,
                 minibatch_size=64):# ← 32   → 64
        self.n=n; self.m=m
        self.gamma=gamma; self.clip_range=clip_range
        ...
        self.model     = AttentionActorCritic(d_model=d_model,n_heads=n_heads,n_layers=n_layers).to(DEVICE)
    ...
    def update(self):
        ...
        B = len(self.buffer)
        drone_feats = torch.tensor(np.array(self.buffer.drone_feats), dtype=torch.float32, device=DEVICE)
        task_feats  = torch.tensor(np.array(self.buffer.task_feats),  dtype=torch.float32, device=DEVICE)
...
def rolling_mean(values, window=50):
    ...
    for i in range(len(values)):
        s=max(0,i-window+1); out.append(float(np.mean(values[s:i+1])))
    return out
...
def print_stats(label, costs, times=None):
    ...
    if times:
        t=np.array(times); print(f"Time  mean ± std  : {t.mean():.2f} ± {t.std():.2f} s")
...
def plot_convergence(history, out_dir, total_episodes):
    ...
    if history["loss"]:
        ...
        plt.figure(figsize=(10,5)); plt.plot(up, rolling_mean(history["loss"],20))
...
def train_attention(model_path="attn_assignment.pt", episodes=10_000,
    ...
    print(f"Attention config → entropy=0.05  clip=0.25  rollout={update_every}  lr=1e-4  mb=64")

    history = {"reward":[], "cost":[], "gap":[], "loss":[]}

    for ep in range(episodes):
        state = env.reset(); done = False; ep_reward = 0.0
        ...
        while not done:
            ...
            agent.buffer.add(df.numpy(), tf.numpy(), action, lp, r, v, done)
            state = ns; ep_reward += r

            if len(agent.buffer) >= update_every:
                m_ = agent.update()
                if m_: history["loss"].append(m_["actor_loss"]+m_["value_loss"]-m_["entropy"])
        ...
        if (ep+1) % 500 == 0:
            print(f"Ep {ep+1:5d} | avg_r={np.mean(history['reward'][-500:]):.3f} | "
...
def eval_sim(model_path, sim_runs=1000, out_dir="outputs/attn",
    ...
    for _ in range(sim_runs):
        ...
        for _ in range(env.n):
            ...
            chosen.append(action); state,_,_,_ = env.step(action)
        ...
        asgn,cm   = assign_tasks(env.drones, env.tasks,
                                  ...
                                  drone_speed=env.drone_speed, weights=env.weights)
...
def run_airsim(model_path, tests=30, out_dir="outputs/attn",
    ...
    for idx in range(tests):
        ...
        attn_cost = sum(env.pair_cost(i,t) for i,t in enumerate(chosen))
        asgn,cm   = assign_tasks(drones, tasks, drone_battery=drone_battery,
        ...
        while time.time()-t0 < 90:
            if all(reached(client,drone_names[di],tasks[chosen[di]],2.5) for di in range(env.n)):
                ok=True; break
            time.sleep(0.25)
