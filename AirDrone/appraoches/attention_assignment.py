"""
attention_assignment.py  (bug-fixed)
========================
Bugs fixed vs previous version:
  FIX 1 – Rollout accumulation:
      ...
      ACROSS episodes and only updates when >= update_every steps.
      Per-episode flush is REMOVED from the inner loop.

  FIX 2 – Mask in update phase:
      During the PPO gradient update, new log_probs were computed WITHOUT
      the action validity mask, corrupting probability ratios and gradients.
      The boolean mask is now stored in the buffer per step and reapplied
      in the update phase before forming the Categorical distribution.

Hyperparameters (unchanged):
  entropy_coef=0.05, clip_range=0.25, update_every=256, lr=1e-4, mb=64
"""
...
def parse_state(flat_state: np.ndarray, n: int, m: int):
    s = flat_state; idx = 0
    ...
    return (torch.tensor(drone_feats, dtype=torch.float32),
...
class AttentionActorCritic(nn.Module):
    def __init__(self, drone_feat_dim=DRONE_FEAT_DIM, task_feat_dim=TASK_FEAT_DIM,
                 d_model=128, n_heads=4, n_layers=3, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.drone_encoder = nn.Sequential(
            nn.Linear(drone_feat_dim, d_model), nn.LayerNorm(d_model), nn.ReLU())
        self.task_encoder  = nn.Sequential(
            nn.Linear(task_feat_dim,  d_model), nn.LayerNorm(d_model), nn.ReLU())
    ...
    def forward(self, drone_feats, task_feats, task_mask=None):
        """
        task_mask: (B, m) bool tensor where True = INVALID action.
                   Applied to logits before forming distribution.
        """
        B, n, _ = drone_feats.shape
        d_emb = self.drone_encoder(drone_feats) + self.drone_type_emb
        t_emb = self.task_encoder(task_feats)   + self.task_type_emb
        tokens = torch.cat([d_emb, t_emb], dim=1)
        tokens = self.transformer(tokens)
        drone_out = tokens[:, :n, :]
        task_out  = tokens[:, n:, :]
        logits = self.actor_head(task_out).squeeze(-1)
        if task_mask is not None:
            logits = logits.masked_fill(task_mask, -1e9)
        value = self.critic_head(drone_out.mean(dim=1)).squeeze(-1)
        return logits, value


# ── Buffer (stores mask per step) ────────────────────────────────────────────

class PPOBuffer:
    """
    Accumulates transitions ACROSS episodes until update_every is reached.
    Stores per-step boolean invalid-action mask for Fix 2.
    """
    ...
    def clear(self):
        ...
        self.values      = []
        self.dones       = []
        self.inv_masks   = []   # FIX 2: (m,) bool, True = INVALID

    def add(self, df, tf, a, lp, r, v, d, inv_mask):
        self.drone_feats.append(df)
        self.task_feats.append(tf)
        self.actions.append(a)
        self.logprobs.append(lp)
        self.rewards.append(r)
        self.values.append(v)
        self.dones.append(d)
        self.inv_masks.append(inv_mask)   # FIX 2
...
class AttentionAgent:
    def __init__(self, n, m, d_model=128, n_heads=4, n_layers=3,
                 lr=1e-4,
                 gamma=0.99,
                 clip_range=0.25,
                 entropy_coef=0.05,
    ...
    def select_action(self, state, valid_actions=None, deterministic=False):
        """
        Returns (action, log_prob, value, inv_mask).
        inv_mask: (m,) bool tensor, True = INVALID — stored in buffer for Fix 2.
        """
        df, tf = self._to_tensors(state)

        # Build invalid-action mask: True = invalid
        inv_mask = torch.ones(self.m, dtype=torch.bool)   # default: all invalid
        if valid_actions is not None:
            inv_mask[valid_actions] = False               # valid → not invalid

        with torch.no_grad():
            logits, value = self.model(df, tf,
                                       inv_mask.unsqueeze(0).to(DEVICE))
            dist   = torch.distributions.Categorical(logits=logits)
            action = torch.argmax(logits, 1) if deterministic else dist.sample()
            lp     = dist.log_prob(action)

        return int(action.item()), float(lp.item()), float(value.item()), inv_mask
    ...
    def update(self):
        ...
        drone_feats = torch.tensor(
            np.array(self.buffer.drone_feats), dtype=torch.float32, device=DEVICE)
        task_feats  = torch.tensor(
            np.array(self.buffer.task_feats),  dtype=torch.float32, device=DEVICE)
        ...
        # FIX 2: stack stored invalid masks → (B, m)
        inv_masks = torch.stack(self.buffer.inv_masks).to(DEVICE)
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, B, bs):
                ...
                # FIX 2: pass per-step invalid mask to model
                mb_inv = inv_masks[mb]                         # (bs, m)
                lg, vp = self.model(drone_feats[mb], task_feats[mb], mb_inv)
...
def print_stats(label, costs, times=None):
    costs = np.array(costs); n = len(costs); se = costs.std() / np.sqrt(n)
...
def plot_convergence(history, out_dir, total_episodes):
    ...
    for key, ylabel, title, fname in [
        ...
        ("gap",    "Avg gap",     "Attention Optimality Gap Convergence", "gap_convergence.png"),
    ]:
    ...
    if history["loss"]:
        ...
        plt.xlabel("Update"); plt.ylabel("Loss"); plt.title("Attention PPO Loss")
...
def train_attention(model_path="attn_assignment.pt", episodes=10_000,
                    update_every=256, out_dir="outputs/attn",
    ...
    save_json(os.path.join(out_dir, "config.json"), {
        "algo": "attention_ppo", "episodes": episodes, "update_every": update_every,
        "entropy_coef": 0.05, "clip_range": 0.25, "lr": 1e-4, "minibatch_size": 64,
    ...
    print(f"Attn config   : entropy=0.05  clip=0.25  rollout={update_every}  lr=1e-4  mb=64")
    print(f"FIX 1 active  : buffer accumulates ACROSS episodes (no per-episode flush)")
    print(f"FIX 2 active  : invalid-action mask stored & reapplied during PPO update")
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # FIX 2: select_action returns invalid mask
            action, lp, v, inv_mask = agent.select_action(
                state, valid_actions=valid, deterministic=False
            )
            df, tf = parse_state(state, n, m)
            ns, r, done, _ = env.step(action)

            # FIX 2: store invalid mask in buffer
            agent.buffer.add(df.numpy(), tf.numpy(), action, lp, r, v, done, inv_mask)
            state = ns
            ep_reward += r

        # FIX 1: update only when buffer has enough cross-episode transitions
...
def eval_sim(model_path, sim_runs=1000, out_dir="outputs/attn",
             task_multiplier=1, weights=None,
             d_model=128, n_heads=4, n_layers=3):
    os.makedirs(out_dir, exist_ok=True)
    env = AssignmentEnv(n=3, task_multiplier=task_multiplier, weights=weights)
...
def run_airsim(model_path, tests=30, out_dir="outputs/attn",
               task_multiplier=1, weights=None,
               d_model=128, n_heads=4, n_layers=3):
    try:
        import airsim
    ...
    for idx in range(tests):
        ...
        print(f"  Attention={attn_cost:.4f}  Hungarian={hun_cost:.4f}  gap={attn_cost-hun_cost:.4f}")
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        train_attention(args.model_path, args.episodes, args.update_every,
